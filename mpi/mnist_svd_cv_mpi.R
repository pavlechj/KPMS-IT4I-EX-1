...
#' 
model_report = function(models, kplot = 0, plot.name = "BasisImages.pdf") {
...
setback("OPENBLAS", quiet = TRUE)
setthreads(blas_threads, quiet = TRUE)
...
my_rows = comm.chunk(nrow(cv), form = "vector")
my_cv_err = mclapply(my_rows, fold_err, cv = cv, folds = folds, train = train,
                  mc.cores = fork_cores)
...
if(comm.rank() == 0) {
  pdf("Crossvalidation.pdf")
    print(ggplot(data.frame(pct = pars, error = cv_err_par/nrow(train)), 
...
## recompute and test with optimal pct
if(comm.rank() == 1) {
